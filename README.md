...
| `year_normalized` | float [0, 1] · `NaN` | Recording year scaled linearly to [0, 1] across the full corpus: 0 = oldest year present (1978), 1 = newest year present (2023). Year is extracted from the `date` column as the first 4-digit sequence, so it handles all mixed date formats. `NaN` only for rows where `date` is absent. |
| `coda_duration_s` | float · `NaN` | Total coda duration in seconds: sum of `ICI1` through `ICI(n_clicks−1)`. Since ICI columns past `n_clicks−1` are `NaN` in the source data, this is a simple column-wise sum with no additional join. `NaN` only for the handful of rows where all ICI values are absent. |
| `morpheme_seq` | str · `NaN` | Morfessor Baseline segmentation of each coda's ICI symbol string, stored as space-joined morphemes (e.g. `"BCC AB"`). Each ICI is first discretised to one of four symbols (A–D) using per-position quantile bin edges computed from the full unified corpus; Morfessor then splits the resulting string into sub-units. The trained model is cached at `data/morfessor_n4.bin`; delete that file to force a retrain. `NaN` for codas outside the 3–40 click range or with missing ICIs. For details on the Morfessor training methodology and morpheme purity analysis see [whale-grammar](https://github.com/morganrivers/whale-grammar). |
